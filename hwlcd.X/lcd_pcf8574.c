...
#define LCD_CLEAR_DISPLAY           0x01u   // Mode : Clears display

#define LCD_RETURN_HOME             0x02u   // Mode : Returns cursor to home posn.

// Entry Mode Set
#define LCD_ENTRY_MODE_SET          0x04u   // Mode : Entry Mode Set, Sets the cursor move dir and specs whether or not to shift the display
#define LCD_INCREMENT               0x02u       // Sub Mode of ENTRY_MODE_SET : Increment DDRAM (I/D), Entry Left
#define LCD_DECREMENT               0x00u       // Sub Mode of ENTRY_MODE_SET : Decrement DDRAM (I/D), Entry Right
#define LCD_SHIFT_ON                0x01u       // Sub Mode of ENTRY_MODE_SET : Shift On  (S), Shift Display when byte written. Display Shift
#define LCD_SHIFT_OFF               0x00u       // Sub Mode of ENTRY_MODE_SET : Shift Off (S), Don't shift display when byte written. Cursor Move

// Display Function
#define LCD_DISPLAY_ON_OFF          0x08u   // Mode : Display On/Off, Sets on/off of all display, Cursor on/off, Cursor Blink on/off
#define LCD_DISPLAY_ON              0x04u       // Sub Mode of DISPLAY_ON_OFF : Puts display on  (D)
#define LCD_DISPLAY_OFF             0x00u       // Sub Mode of DISPLAY_ON_OFF : Puts display off (D)
#define LCD_CURSOR_ON               0x02u       // Sub Mode of DISPLAY_ON_OFF : Puts cursor on   (C)
#define LCD_CURSOR_OFF              0x00u       // Sub Mode of DISPLAY_ON_OFF : Puts cursor off  (C)
#define LCD_BLINKING_ON             0x01u       // Sub Mode of DISPLAY_ON_OFF : Blinking cursor  (B)
#define LCD_BLINKING_OFF            0x00u       // Sub Mode of DISPLAY_ON_OFF : Solid cursor     (B)

// Display Control
#define LCD_MV_CUR_SHIFT_DISPLAY    0x10u   // Mode : Move the cursor and shifts the display
#define LCD_DISPLAY_SHIFT           0x08u       // Sub Mode of CURSOR_SHFT_DIS : Display shifts after char print   (SC)
#define LCD_CURSOR_SHIFT            0x00u       // Sub Mode of CURSOR_SHFT_DIS : Cursor shifts after char print    (SC)
#define LCD_SHIFT_RIGHT             0x04u       // Sub Mode of CURSOR_SHFT_DIS : Cursor or Display shifts to right (RL)
#define LCD_SHIFT_LEFT              0x00u       // Sub Mode of CURSOR_SHFT_DIS : Cursor or Display shifts to left  (RL)

// Function Set
#define LCD_FUNCTION_SET            0x20u   // Mode : Set the type of interface that the display will use
#define LCD_INTF8BITS               0x10u       // Sub Mode of FUNCTION_SET : Select 8 bit interface         (DL)
#define LCD_INTF4BITS               0x00u       // Sub Mode of FUNCTION_SET : Select 4 bit interface         (DL)
#define LCD_TWO_LINES               0x08u       // Sub Mode of FUNCTION_SET : Selects two char line display  (N)
#define LCD_ONE_LINE                0x00u       // Sub Mode of FUNCTION_SET : Selects one char line display  (N)
#define LCD_FONT_5_10               0x04u       // Sub Mode of FUNCTION_SET : Selects 5 x 10 Dot Matrix Font (F)
#define LCD_FONT_5_7                0x00u       // Sub Mode of FUNCTION_SET : Selects 5 x 7 Dot Matrix Font  (F)

#define LCD_CG_RAM_ADDRESS          0x40u        // Mode : Enables the setting of the Char Gen (CG) Ram Address, to be or'ed with require address
#define LCD_CG_RAM_ADDRESS_MASK     0b00111111u  // Used to mask off the lower 6 bits of valid CG Ram Addresses

#define LCD_DD_RAM_ADDRESS          0x80u        // Mode : Enables the setting of the Display Data (DD) Ram Address, to be or'ed with require address
#define LCD_DD_RAM_ADDRESS_MASK     0b01111111u    // Used to mask off the lower 6 bits of valid DD Ram Addresses
...
#define Bl 0b00001000u  // Backlight enable bit (On = 1, Off =0)
#define En 0b00000100u  // Enable bit (Enable on low edge)
#define Rw 0b00000010u  // Read/Write bit (Read = 1, Write = 0)
#define Rs 0b00000001u  // Register select bit (Data = 1, Control = 0)

// Change here for your I2C to 16 pin parallel interface // TODO Adapt
#define LCD_INIT 0b00000100u // ((0b00000000u | En) & ~Rs) & (~Rw) // Used to set all the O/Ps on the PCF8574 to initialise the LCD
#define LCD_8BIT_INIT 0b00110000u // Used to initialise the interface at the LCD
#define LCD_4BIT_INIT 0b00100000u // Used to initialise the interface at the LCD

#define LCD_PCF8574_ADDR         0x4Eu  // Modify this if the default address is altered
#define LCD_PCF8574_WEAK_PU      0b11110000u // Used to turn on PCF8574 Bits 7-4 on. To allow for read of LCD.

#define LCD_BUSY_FLAG_MASK       0b10000000u // Used to mask off the status of the busy flag
#define LCD_ADDRESS_COUNTER_MASK 0b01111111u // Used to mask off the value of the Address Counter
#define LCD_MAX_COLS             20
#define LCD_MAX_ROWS             4

#define LCD_LINE1 0x00u	// Constant used to point to start of LCD Line 1
#define LCD_LINE2 0x40u   // Constant used to point to start of LCD Line 2
#define LCD_LINE3 0x14u   // Constant used to point to start of LCD Line 3
#define LCD_LINE4 0x54u	// Constant used to point to start of LCD Line 4
...
{
  unsigned char row_offsets[] = { LCD_LINE1, LCD_LINE2, LCD_LINE3, LCD_LINE4 };
  if ( row >= _numlines ) {
...
void LCDcreateChar(unsigned char location, unsigned char charmap[]) {
    ...
    LCDcommandWrite(LCD_CG_RAM_ADDRESS | (location << 3u));
    for (int i=0; i<8; i++)
...
static void LCDsend(unsigned char value, unsigned char RsMode) {
    unsigned char highnib = value & 0xF0u;

    unsigned char lownib  = value << 4u;
    lownib &= 0xF0;
...
static void LCDwritePCF8574(unsigned char value) {
    i2c_write_byte(LCD_PCF8574_ADDR, value | _backlightval);
}


static unsigned char LCDreadPCF8574(void) {
    return i2c_read_byte(LCD_PCF8574_ADDR);
}
